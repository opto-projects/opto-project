	...
	ui(new Ui::OCTproZ)
{
	...
	qApp->setApplicationVersion(QString("%1.%2.%3").arg(APP_VER_MAJOR).arg(APP_VER_MINOR).arg(APP_VER_PATCH));
...
void OCTproZ::setSystem(QString systemName) {
	...
	if(system == nullptr){
		emit error(tr("Opening of the system failed. Could not find a system with the name: ") + systemName);
		return;
	}
	...
	this->setWindowTitle(APP_NAME +  tr(" - ") + systemName);
