...
class AxialPsfAnalyzerPlugin : public Plugin
{
	Q_OBJECT
...
public:
	AxialPsfAnalyzerPlugin(QObject* parent = nullptr);
	~AxialPsfAnalyzerPlugin();

	InterfaceBase* getInterfaceInstance();
};
...
class AxialPsfAnalyzer : public Extension
{
	...
	friend class AxialPsfAnalyzerPlugin;
public:
	AxialPsfAnalyzer(QObject* parent = nullptr) ;
	~AxialPsfAnalyzer();
...
signals:
	void newFrame(void* frame, unsigned int bitDepth, unsigned int width, unsigned int height, unsigned int channel);
