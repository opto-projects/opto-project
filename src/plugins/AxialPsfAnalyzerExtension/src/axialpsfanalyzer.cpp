...
AxialPsfAnalyzerPlugin::AxialPsfAnalyzerPlugin(QObject* parent ) : Plugin(parent)
{
	type = PLUGIN_TYPE::EXTENSION;
	name = "AxialPsfAnalyzer";
	descr = "FWHM measurement of axial point spread function";
}
AxialPsfAnalyzerPlugin::~AxialPsfAnalyzerPlugin()
{
}

InterfaceBase* AxialPsfAnalyzerPlugin::getInterfaceInstance() {
	NEW_PLUGININSTANCE(AxialPsfAnalyzer)
}

AxialPsfAnalyzer::AxialPsfAnalyzer(QObject* parent ) :
	Extension(parent),
...
void AxialPsfAnalyzer::activateExtension() {
	//this method is called by system as soon as user activates the extension. If the extension controls hardware components, they can be prepared, activated, initialized or started here.
	this->active = true;
}

void AxialPsfAnalyzer::deactivateExtension() {
	//this method is called by system as soon as user deactivates the extension. If the extension controls hardware components, they can be deactivated, resetted or stopped here.
	this->active = false;
}

void AxialPsfAnalyzer::settingsLoaded(QVariantMap settings) {
	//this method is called by system and provides a QVariantMap with stored settings/parameters.
...
void AxialPsfAnalyzer::processedDataReceived(void* buffer, unsigned int bitDepth, unsigned int samplesPerLine, unsigned int linesPerFrame, unsigned int framesPerBuffer, unsigned int buffersPerVolume, unsigned int currentBufferNr) {
	if(this->active){
		if((!this->isCalculating && this->processedGrabbingAllowed && this->autoFetch) || (this->singleFetch && this->processedGrabbingAllowed)){
			...
			emit newFrame(this->frameBuffersProcessed[this->copyBufferId], bitDepth, samplesPerLine, linesPerFrame,1);
