...
namespace MM {
   ...
   /**
    * State device API, e.g. filter wheel, objective turret, etc.状态设备接口，如滤光轮、物镜转盘等。
    */
   ...
   /**
    * ADC and DAC interface.信号输入输出接口，支持电压设置和序列化操作。
    */
   ...
   /**
   * Devices that can change magnification of the cameraworker
   */
    ...
    * widely used implmentation is the GenericSLM. 空间光调制器接口，支持图像加载、显示、序列化操作等。
    */
    ...
    * and there is also no support for waveforms. 振镜接口，支持点扫描、多边形绘制等功能。
    * 
    */
    ...
    * and managing auto-configuration (discovery) of other devices.集线器接口，用于管理复杂设备及其子设备。
    */
   ...
   public:
       ...
       * general cameraworker has been set to output debug logging.
       */
      ...
      /**
       * When the new image has ready, use this callback to inform the UI
       */
      virtual int OnNewImageReady(const MM::Device* device, int imagenumber) = 0;
