...
void BeamConfig::load(QSettings *s)
{
...
void BeamConfig::save(QSettings *s, bool compact) const
{
    SAVE(plot.normalize);
    SAVE(plot.rescale);
    SAVE(plot.fullRange);
    SAVE(plot.customScale.on);
    if (!compact || plot.customScale.on) {
    ...
    SAVE(roiMode);
    if (!compact || roiMode == ROI_SINGLE) {
        SAVE(roi.left);
        SAVE(roi.top);
        SAVE(roi.right);
        SAVE(roi.bottom);
    }
    if (!compact || roiMode == ROI_MULTI) {
