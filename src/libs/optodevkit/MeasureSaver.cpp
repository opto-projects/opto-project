...
{
    s = Settings::getInstance()->getQSettings();
...
QString MeasureSaver::start(const MeasureConfig &cfg, AcquisitionSystem*cam)
{
    if (!cfg.durationInf) {
    ...
    out << "Index"
        << SEP << "Timestamp";
    if (cam->getRoiMode() == ROI_MULTI) {
        _multires_cnt = cam->getRois().size();
        for (int i = 0; i < _multires_cnt; i++) {
            const auto &roi = cam->getRois().at(i);
...
public:
    MesureSaverDlg(const MeasureConfig &cfg) {
        ...
        labDuration->setForegroundRole(QPalette::Light);
        ...
        labImgInterval->setForegroundRole(QPalette::Light);
    ...
    void exportConfig()
    {
        ...
        if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            ...
            //emit error(tr("Failed to create configuration file:\n%1").arg(f.errorString()));
            return;
        }
        f.close();
    ...
    {
        QString name = inputText(tr("Preset name:"), cbPresets->currentText());
        name = name.trimmed();
        if (name.isEmpty())
            return;
        QSettings s;
        ...
        if (s.contains(name)) {
            if (!yes(tr("Preset with this name already exists, overwrite?")))
                return;
        }
    ...
    void deleteConfigPreset()
    {
        ...
        if (name.isEmpty()) {
            warning(tr("Please select a preset in the drop-down"));
            return;
        }
        if (!yes(tr("Preset will be deleted:<p><b>%1</b><p>Confirm?").arg(name)))
            return;
