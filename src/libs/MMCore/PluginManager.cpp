...
#ifdef _WIN32
const char* const LIB_FOLDER = "devices\\";
#else
const char* const LIB_FOLDER = "libmmgr_dal_";
#endif
...
CPluginManager::GetDeviceAdapter(const std::string& moduleName)
{
   ...
   std::string filename(LIB_FOLDER);
   filename += moduleName;
...
CPluginManager::GetModules(std::vector<std::string> &modules, const char* searchPath)
{
   ...
   if (hSearch != -1L) // Match found
   {
      do {
         ...
         strippedName = std::string(moduleFile.name).substr(0, strippedName.find_first_of("."));
