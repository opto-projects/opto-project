...
int CoreCallback::OnPropertiesChanged(const MM::Device* /* caller */)
{
   ...
   // TODO It is inconsistent that we do not update the cameraworker state cache in
...
int CoreCallback::OnNewImageReady(const MM::Device* device, int imagenumber)
{
    if (core_->externalCallback_) {
        char label[MM::MaxStrLength];
        device->GetLabel(label);
        core_->externalCallback_->onNewImageReady(label, imagenumber);
    }
    return DEVICE_OK;
}
