...
//-----------------------------------------------------------------------------
// DESCRIPTION:   A camera implementation that is backed by the file cameraworker
...
const unsigned char* FakeCamera::GetImageBuffer()
{
	...
	for (int i = 0; i < roi_.cols * roi_.rows; i++) {
		memcpy(img_.GetPixelsRW() + i, roi_.data + srcOffset, 1);
...
ERRH_START
	...
	MMThreadGuard g(imgPixelsLock_);
	img_.Resize(width_, height_, byteCount_);
...
void FakeCamera::getImg() const
{
	...
	cv::Mat img = path == lastFailedPath_ ? lastFailedImg_ : cv::imread(path, -1);
...
void FakeCamera::GenerateEmptyImage(ImgBuffer& img)
{
	MMThreadGuard g(imgPixelsLock_);
