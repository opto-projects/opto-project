...
//                cameraworker without the need to connect to the actual hardware. 
//                
...
int CDemoCamera::InsertImage()
{
   ...
   Metadata md;
   md.put("Camera", label);
   string sst = CDeviceUtils::ConvertToString(sequenceStartTime_.getMsec());
   //md.put(MM::g_Keyword_Metadata_StartTime, CDeviceUtils::ConvertToString(sequenceStartTime_.getMsec()));//g_Keyword_Metadata_StartTime is removed
...
/**
 * Function that converts between the Galvo and Camera coordinate cameraworker
 */
