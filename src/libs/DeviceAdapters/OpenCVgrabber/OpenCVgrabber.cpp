...
// External names used used by the rest of the cameraworker
// to load particular device from the "DemoCamera.dll" library
const char* g_CameraDeviceName = "OpenCVgrabber";
const char* cIDName = "ID";
...
* If this device used as intended within the Micro-Manager cameraworker,
...
int COpenCVgrabber::Initialize()
{
   ...
   capture_ = cv::VideoCapture(cameraID_);

   if (!capture_.isOpened()) // do we have a capture_ device?
   {
...
#else
   long w = (long)capture_.get(cv::CAP_PROP_FRAME_WIDTH);
   long h = (long)capture_.get(cv::CAP_PROP_FRAME_HEIGHT);
#endif
   ...
   // CameraName
   nRet = CreateProperty(MM::g_Keyword_CameraName, "uvc camera", MM::String, true);
   ...
   // exposure SetExposure GetExposure设置属性 OnExposure设置相机 
   pAct = new CPropertyAction(this, &COpenCVgrabber::OnExposure);
   nRet = CreateProperty(MM::g_Keyword_Exposure,  CDeviceUtils::ConvertToString(GetExposure()), MM::Float, false, pAct);
   assert(nRet == DEVICE_OK);
   SetPropertyLimits(MM::g_Keyword_Exposure, -13, 0);
   //Windows – exposure times  index ranges typically from 0 to -13 
   //The equation is  EXP_TIME = 2^(-EXP_VAL) 

   // gain
   pAct = new CPropertyAction(this, &COpenCVgrabber::OnGain);
   nRet = CreateProperty(MM::g_Keyword_Gain, "0", MM::Float, false, pAct);
   assert(nRet == DEVICE_OK);
   SetPropertyLimits(MM::g_Keyword_Gain, -1, 10);
   ...
   CreateProperty("FlipX","0",MM::Integer,false,pAct);
   SetPropertyLimits("FlipX",0,1);

   pAct = new CPropertyAction (this, &COpenCVgrabber::OnFlipY);
   CreateProperty("FlipY","0",MM::Integer,false,pAct);
   SetPropertyLimits("FlipY",0,1);
...
const unsigned char* COpenCVgrabber::GetImageBuffer()
{
   ...
   if (pixelType.compare(g_PixelType_24bitRGB) == 0)
   {
      /*
      ...
      */
      ...
      if(roiX_ == 0 && roiY_ == 0){
         RGB3toRGB4((const char*)frame_.data, (char *) img_.GetPixelsRW(), frame_.cols, frame_.rows);
      } else {
          cv::Rect roiRect(roiX_, roiY_, img_.Width(), img_.Height());
          cv::Mat ROI = frame_(roiRect);

         if(!ROI.empty()) return 0; 
         RGB3toRGB4((const char*) ROI.data, (char *) img_.GetPixelsRW(), ROI.cols, ROI.rows);
      ...
      for(int i=0; i < frame_.cols * frame_.rows; i++){
         memcpy(img_.GetPixelsRW()+i, frame_.data +srcOffset,1);
         srcOffset += 1;//srcOffset += 3;
      } 
   }
...
int COpenCVgrabber::InsertImage()
{
   ...
   if (ret == DEVICE_OK)
   {
       GetCoreCallback()->OnNewImageReady(this, 1);
   }
   return ret;
}
...
int COpenCVgrabber::OnExposure(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    int ret = DEVICE_ERR;
    switch (eAct)
    {
    case MM::AfterSet:
    {
        ...
        double autoexp = capture_.get(cv::CAP_PROP_AUTO_EXPOSURE);//always -1?
        
        //if (autoexp < 0) //== -1 // 注意启用自动曝光参数是-1
        //{
        //    capture_.set(cv::CAP_PROP_AUTO_EXPOSURE, 0.25);// 注意禁用的参数是0.25
        //}
        //autoexp = capture_.get(cv::CAP_PROP_AUTO_EXPOSURE);
        if (!capture_.set(cv::CAP_PROP_EXPOSURE, exp))
        {
            return DEVICE_ERR;
        }
        ret = DEVICE_OK;
    ...
    case MM::BeforeGet:
    {
        ...
        double exp = capture_.get(cv::CAP_PROP_EXPOSURE); // try to get the exposure from OpenCV - not all drivers allow this
        if (exp >= 1)
            return DEVICE_ERR;
        ret = DEVICE_OK;
        pProp->Set((double)exp);
    }break;
    }
    return ret;
}
   ...
   case MM::AfterSet:
      {
         ...
         if (pixelType.compare(g_PixelType_8bit) == 0)
         {
            ...
            bitDepth_ = 8;
            double c = capture_.get(cv::CAP_PROP_MONOCHROME);
            capture_.set(cv::CAP_PROP_MONOCHROME, 1);
            c = capture_.get(cv::CAP_PROP_MONOCHROME);
   ...
   case MM::AfterSet:
      {
       ...
       pProp->Get(resolution);
         ...
         cameraCCDXSize_ = capture_.get(cv::CAP_PROP_FRAME_WIDTH);
         cameraCCDYSize_ = capture_.get(cv::CAP_PROP_FRAME_HEIGHT);
